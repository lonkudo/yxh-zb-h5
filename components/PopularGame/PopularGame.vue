<template>
	<view class="margin-xs b-f br-5">
		<scroll-view scroll-x="true" >
			<view class="flex game-swiper padding-top-sm padding-bottom-sm justify-between" >
				<view class="game-item flex flex-direction justify-between padding-left-sm padding-right-sm" v-for="(item,index) in gameList" :key="index">
						<view class="top flex align-center justify-between">
							<text class="fc-b-9 fs-22 f-hide w-230 ">{{item.competition_name}}</text>
							<view v-if="true">
							<text class="fc-b-9 fs-22 margin-left-xs">{{item.match_time |  formatGiven('MM-dd hh:ss')}}</text>
							</view>
							<view v-else>
