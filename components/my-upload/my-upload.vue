<template>
	<view class="my-upload" v-if="!disabled">
		<view
			v-if="showUploadList"
		...
		>
			...
			<u-line-progress
				v-if="
					showProgress && item.progress > 0 && item.progress != 100 && !item.error
				"
				:show-percent="false"
				height="16"
				class="u-progress"
...
<script>
	 ...
	 * @example <my-upload :action="action" :file-list="fileList" ></my-upload>
	 */
	export default {
		name: 'my-upload',
		props: {
		...
		methods: {
					...
					.then((res) => {
						...
						res.tempFiles.map((val, index) => {
							...
							if (val.size > maxSize) {
								...
								this.showToast("The file's size is too big.")
							} else {
								if (maxCount <= lists.length) {
									this.$emit('on-exceed', val, this.lists, this.index)
									this.showToast('Maxmium size exceeded.')
									return
								}
								lists.push({
									url: val.path,
			...
			async uploadFile(index = 0) {
				...
				// 检查上传地址
				if (!this.action) {
					this.showToast('Please check yout upload action address', true)
					return
				}
				this.lists[index].error = false
				this.uploading = true
			...
			deleteItem(index) {
				uni.showModal({
					title: 'Notice',
					content: 'Comfirm to delete this file.',
					success: async (res) => {
						if (res.confirm) {
			...
			checkFileExt(file) {
				...
				if (!noArrowExt)
					this.showToast(`Can not choose file with ${fileExt} suffix`)
				return noArrowExt
			},
		},
	}
